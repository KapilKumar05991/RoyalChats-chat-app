...
function GroupBar({ group }: GroupProp) {
    const setGroup = useChatStore(state => state.setGroup)
    ...
    return (
        <div className="hover:bg-white/10 flex items-center rounded-md p-2 gap-2">
            <Avatar className="cursor-pointer border-2 border-primary z-30 relative size-9">
            ...
            <div onClick={handleClick} className="flex-1 flex flex-col">
                ...
                {info.count > 0 ?
                    <p className="font-bold text-xs flex justify-between">
                        ...
                        <Badge className="h-5 bg-green-500 font-semibold min-w-5 rounded-full px-1 font-mono tabular-nums">{info.count < 101 ? info.count : '100+'}</Badge>
                    </p> : ''
                }
            </div>
        </div>
